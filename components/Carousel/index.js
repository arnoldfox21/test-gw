  ...
  return (
    <div className="sm:p-16 p-2 pt-8 sm:pr-0 relative">
        ...
        <div className="sm:w-4/6 w-full h-full relative">
          ...
          >
            {activeItem?.timeline?.map((data, i) => (
              <div key={i} className="space-y-4 flex-none rounded-xl w-full flex flex-col items-center justify-center">
                <img
                  src={data?.image}
      ...
      <div className="w-full flex justify-end z-10 sm:-mt-[44px] mt-28">
        <div className="bg-[#4d73b6] rounded-xl bg-opacity-70 min-h-56 sm:w-4/6 w-full text-white sm:p-20 p-4 sm:pr-32">
          <div className="font-bold text-2xl pb-4">{activeItem?.timeline[scrollData.index]?.title}</div>
          <div className="text-xl">{activeItem?.timeline[scrollData.index]?.description}</div>
        </div>
      </div>
    </div>
  );
